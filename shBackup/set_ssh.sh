...
if [ ! -f "$PRIV_KEY_FILE" ]; then
  ...
  mkdir -p ~/.ssh
  chmod 700 ~/.ssh
  ssh-keygen -t rsa -b 4096 -N "" -f "$PRIV_KEY_FILE"
else
...
if ! grep -Fxq "$PUB_KEY_CONTENT" "$AUTHORIZED_KEYS"; then
  echo "$PUB_KEY_CONTENT" >> "$AUTHORIZED_KEYS"
  echo -e "\033[1;32m公钥已添加。\033[0m"
else
  echo -e "\033[1;33m公钥已存在，跳过添加。\033[0m"
fi
